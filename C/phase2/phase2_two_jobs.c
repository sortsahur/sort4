...
void setpagetable(int pn, int fn) {
    int e = PTR + pn;
    mem[e][0] = '1'; mem[e][1] = ' ';
...
void readcard() {
    ...
    if (ra == -1) { terminateprog("Invalid Page Fault"); IC = 9999; return; }
    int i = 0, m = 0;
...
void mastermode() {
    ...
    else if (PI) terminateprog("Error");
}
...
void executeprog() {
    while (1) {
        ...
        if (ra == -1) { terminateprog("Invalid Page Fault"); break; }
        ...
        if (!strcmp(op,"GD")) { SI=1; IC++; TLC++; mastermode(); }
            ...
            IC++; TLC++;
            int a = addressmap((IR[2]-'0')*10+(IR[3]-'0'));
            if (a==-1) { terminateprog("Invalid Page Fault"); break; }
...
void load() {
    ...
    while (fgets(line, 80, inputfp)) {
        ...
        if (!strcmp(word,"$AMJ")) {
            ...
            printf("Loading Job %d (TTL=%d TLL=%d)\n", jobcount, TTL, TLL);
        }
